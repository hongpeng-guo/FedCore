...
from .trainer_creator import create_model_trainer
from .client import Client
from collections import defaultdict
from time import time


class FedCoreAPI(object):
    def __init__(self, args, device, dataset, model):
        self.recorder = defaultdict(lambda: {})
        self.recorder_file = "{}/{}_{}.log".format(
            args.record_dir, args.record_file, args.stragglers_percent
        )
        self.device = device
        self.args = args
        [
        ...
        self.compute_power = 1.0 + self.args.data_compute_correlation * 0.5 / np.std(
            train_data_local_num
        ) * (train_data_local_num - train_data_local_num.mean())
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round + 1):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                # determin if the training size is too large that coreset generatoin
                # is not affrdable
                if (
                    self.epoch_ddl * client.compute_power * self.args.epochs
                    < client.local_sample_number
                ):
                    continue
                # init epoch training with full data
                init_train_s = time()
                w = client.train_init_epoch(copy.deepcopy(w_global))
                init_train_f = time()

                # Note: A better implementation of coreset generation can reduce the
                # time overhead to almost zero which use free grads from initial epoch
                if round_idx >= 0:
                    client.update_training_coreset(self.epoch_ddl)

                # train on new dataset
                train_s = time()
                w = client.train_remaining_epochs(copy.deepcopy(w))
                train_f = time()
                if "train" not in self.recorder[round_idx]:
                    self.recorder[round_idx]["train"] = {}
                self.recorder[round_idx]["train"][idx] = (
                    init_train_f - init_train_s + train_f - train_s
                )
    ...
    def _local_test_on_all_clients(self, round_idx):
        ...
        for client_idx in range(self.args.client_num_in_total):
            ...
            # test data
            if self.args.dataset == "cifar10" and client_idx > 0:
                continue
    ...
    def _local_test_on_validation_set(self, round_idx):
        ...
        client = self.client_list[0]
        client.update_local_dataset(
            0, None, self.val_global, None, self.compute_power[0]
        )
        # test data
