...
def load_cifar10_data(
    ...
    if process_id != 0:
        is_download = (
            False
            if (len(synthetic_data_url) != 0 or len(private_local_data) != 0)
            else True
        )

    if data_efficient_load:
        cifar10_train_ds = CIFAR10(
            datadir, train=True, download=True, transform=train_transform
        )
        cifar10_test_ds = CIFAR10(
            datadir, train=False, download=True, transform=test_transform
        )
    else:
        cifar10_train_ds = CIFAR10_truncated(
            datadir, train=True, download=is_download, transform=train_transform
        )
        cifar10_test_ds = CIFAR10_truncated(
            datadir, train=False, download=is_download, transform=test_transform
        )
...
def partition_data(
    ...
    (
        ...
        cifar10_train_ds,
        cifar10_test_ds,
    ) = load_cifar10_data(datadir, process_id, synthetic_data_url, private_local_data)
    ...
    if partition == "homo":
        ...
        while min_size < 10:
            print(min_size)
            net_size = np.random.gamma(alpha, 1, n_nets)
            net_size = np.sort(N * (net_size / np.sum(net_size))).astype(int).tolist()
            for i in range(N - sum(net_size)):
                net_size[i] += 1
            min_size = min(net_size)
        ...
        for k in range(K):
            ...
            proportions = np.sort(np.random.dirichlet(np.repeat(1.5, n_nets)))
            class_size = (proportions * len(idx_k)).astype(int)
            class_size[-1] += len(idx_k) - np.sum(class_size)
            for itr in range(2 * n_nets):
                idx = itr % n_nets
                overflow = len(idx_batch[idx]) + class_size[idx] - net_size[idx]
                if overflow > 0:
                    class_size[idx] = net_size[idx] - len(idx_batch[idx])
                    class_size[(idx + 1) % n_nets] += overflow
            class_size = np.cumsum(class_size)[:-1]
            idx_batch = [
                idx_j + idx.tolist()
                for idx_j, idx in zip(idx_batch, np.split(idx_k, class_size))
            ]
