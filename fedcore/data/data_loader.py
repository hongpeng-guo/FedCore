...
from .stackoverflow_nwp.data_loader import (
    load_partition_data_federated_stackoverflow_nwp,
)
from .synthetic_0_0.data_loader import load_partition_data_federated_synthetic_0_0
from .synthetic_05_05.data_loader import load_partition_data_federated_synthetic_05_05
from .synthetic_1_1.data_loader import load_partition_data_federated_synthetic_1_1
...
def setup_s3_service(s3_config):
    ...
    s3 = boto3.client(
        "s3",
        region_name=s3_config["CN_REGION_NAME"],
        aws_access_key_id=s3_config["CN_S3_AKI"],
        aws_secret_access_key=s3_config["CN_S3_SAK"],
        config=_config,
    )
...
def data_server_preprocess(args):
    ...
    if args.process_id == 0:
        ...
        if args.dataset == "cifar10":
            ...
            private_local_dir, split_status, edgeids, dataset_s3_key = check_rundata(
                args
            )
            ...
            # MLOPS Run need to Split Data
                ...
                if split_status == 0 or split_status == 3:
                    ...
                    logging.info(
                        "Data Server Is Splitting Dataset, Waiting For Mqtt Message"
                    )
                elif split_status == 2:
...
def check_rundata(args):
    ...
    try:
        ...
        return (
            response.json()["private_local_dir"],
            response.json()["split_status"],
            response.json()["edgeids"],
            response.json()["dataset_s3_key"],
        )
    except requests.exceptions.SSLError as err:
        print(err)
...
def load_synthetic_data(args):
    if (
        args.training_type == "cross_silo"
        and args.dataset == "cifar10"
        and hasattr(args, "synthetic_data_url")
        and args.synthetic_data_url.find("https") != -1
    ):
    ...
    if dataset_name == "mnist":
        ...
        if dataset_name == "cifar10":
            if hasattr(args, "synthetic_data_url") or hasattr(
                args, "private_local_data"
            ):
        ...
        elif dataset_name == "synthetic_0_0":
            data_loader = load_partition_data_federated_synthetic_0_0
        elif dataset_name == "synthetic_0.5_0.5":
            data_loader = load_partition_data_federated_synthetic_05_05
        elif dataset_name == "synthetic_1_1":
            data_loader = load_partition_data_federated_synthetic_1_1
        else:
            data_loader = load_partition_data_cifar10
        (
            train_data_num,
