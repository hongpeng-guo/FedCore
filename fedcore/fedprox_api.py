...
from .trainer_creator import FedProxModelTrainer
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round + 1):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                # determin training epochs if train partial epochs
                epochs = self.args.epochs
                if self.epoch_ddl * client.compute_power < client.local_sample_number:
                    epochs = int(
                        self.epoch_ddl
                        * client.compute_power
                ...
                train_s = time()
                w = client.train_num_epochs(copy.deepcopy(w_global), epochs)
                train_f = time()
                if "train" not in self.recorder[round_idx]:
                    self.recorder[round_idx]["train"] = {}
                self.recorder[round_idx]["train"][idx] = train_f - train_s
                mlops.event(
                    "train",
                    event_started=False,
                    event_value="{}_{}".format(str(round_idx), str(idx)),
